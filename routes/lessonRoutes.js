...
function processStepForFrontend(step, index) {
  ...
  if (step.type === 'exercise' && !step.gameType) {
    if (Array.isArray(step.data) && step.data.length > 0) {
      // exercises already in data array
    ...
    const processedSteps = steps.map((step, index) => {
      ...
      try {
        switch (stepType) {
          ...
          // NEW: Interactive step types processing
          case 'data_analysis':
            processedData = {
              prompt: step.prompt || step.data?.prompt || '',
              numericKey: step.data?.numericKey || step.numericKey || 'value',
              numericLabel: step.data?.numericLabel || step.numericLabel || 'Value',
              data: step.data?.data || step.data || [],
              correctAnswer: step.data?.correctAnswer || step.correctAnswer || 0,
              tolerance: step.data?.tolerance || step.tolerance || 1
            };
            break;
          ...
          case 'fraction_visual':
            processedData = {
              prompt: step.prompt || step.data?.prompt || '',
              targetNumerator: step.data?.targetNumerator || step.targetNumerator || 1,
              targetDenominator: step.data?.targetDenominator || step.targetDenominator || 2,
              userTotalBlocks: step.data?.userTotalBlocks || step.userTotalBlocks || 8,
              requiredShaded: step.data?.requiredShaded || step.requiredShaded || 4
            };
            break;
          ...
          case 'geometry_poly':
            processedData = {
              prompt: step.prompt || step.data?.prompt || '',
              shapes: step.data?.shapes || step.shapes || ['Line', 'Circle', 'Square'],
              correctShape: step.data?.correctShape || step.correctShape || 'Circle'
            };
            break;
          ...
          case 'chem_mixing':
            processedData = {
              prompt: step.prompt || step.data?.prompt || '',
              substances: step.data?.substances || step.substances || [],
              tolerance: step.data?.tolerance || step.tolerance || 5,
              successColor: step.data?.successColor || step.successColor || '#22c55e',
              failColor: step.data?.failColor || step.failColor || '#ef4444'
            };
            break;
          ...
          case 'chem_matching':
            processedData = {
              prompt: step.prompt || step.data?.prompt || '',
              pairs: step.data?.pairs || step.pairs || []
            };
            break;
          ...
          case 'english_sentence_fix':
            processedData = {
              prompt: step.prompt || step.data?.prompt || '',
              originalSentence: step.data?.originalSentence || step.originalSentence || '',
              tokens: step.data?.tokens || step.tokens || [],
              errors: step.data?.errors || step.errors || []
            };
            break;
          ...
          case 'english_sentence_order':
            processedData = {
              prompt: step.prompt || step.data?.prompt || '',
              correctOrder: step.data?.correctOrder || step.correctOrder || [],
              scrambledOptions: step.data?.scrambledOptions || step.scrambledOptions || null
            };
            break;
          ...
          case 'language_noun_bag':
            processedData = {
              prompt: step.prompt || step.data?.prompt || '',
              words: step.data?.words || step.words || [],
              targetPos: step.data?.targetPos || step.targetPos || 'noun'
            };
            break;
